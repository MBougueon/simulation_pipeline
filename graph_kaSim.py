...
def sensibility_3d_graph(folder, output, data_measure, graph_format, variables):
    ...
    for i in range(len(concentrat)):
        ...
        xaxix = [i]*len(df_graph[data_measure])
        # df = df[df['TGFB1_by_cells'] != val]
   ...
   # ax.set_xticks(np.log(concentration))
    ...
    ax.set_xticklabels(concentrat, rotation = -10)

    time_ticks = np.arange(0, 200, 50)
    ax.set_yticks(time_ticks)
    ax.set_yticklabels(time_ticks)
    data_measure_ticks = np.arange(0, 36, 2)#to change for other species
    ...
    ax.xaxis.labelpad = 10
        # Show the plot.
    ax.invert_xaxis()
    # ax.set_xlabel('TGFB1 concentration (molecules/cell).10^3', fontsize=8 )
    ax.set_xlabel('Proportion of react_MFB Inactivated', fontsize=8 )
    ax.set_ylabel('Time in days', fontsize=8 )
    ax.set_zlabel(f"{data_measure} occurences", fontsize=8 )
    # plt.show()
